...
def dijkstra_step_by_step(graph, start):
    ...
    # Initial state (Step 0) - This will be yielded first
    yield step_count, set(distances.keys()), dict(distances), dict(previous) # Yield copies, include all nodes in initial distances state

    while queue:
        # Find the node with the minimum distance estimate among nodes not in N'
        # (This is handled efficiently by the heapq)
        ...
        # Process neighbors first to update distances/previous before yielding the state
        ...
        # Now yield the state after processing the current node and its neighbors
    ...
    # This case is handled by the yields inside the loop for reachable nodes.
    # For unreachable nodes, their distance remains 'inf' and predecessor '-'
...
class NetworkApp:
    ...
    def show_dijkstra_steps(self):
        ...
        # These widths are approximate and might need fine-tuning based on actual node names
        col_widths = {'step': 5, 'N\'': 10}
        d_p_col_width = 18 # Adjusted width for D(),p() columns

        # Construct the header row dynamically
        header_line1 = f"{'Step':<{col_widths['step']}}{'N\'':<{col_widths['N\'']}}"
        header_line2 = f"{'':<{col_widths['step']}}{'':<{col_widths['N\'']}}" # Second line for D/p labels

        for node in nodes:
             col_header = f"D({node}),p({node})"
             header_line1 += f"{col_header:<{d_p_col_width}}"
             # Add a separator line for the header bottom
             header_line2 += "-" * (d_p_col_width - 1) + " " # Add separator under each column

        header_separator = "-" * (col_widths['step'] + col_widths['N\''] + len(nodes) * d_p_col_width)

        self.output_text.insert(tk.END, header_line1 + "\n")
        # self.output_text.insert(tk.END, header_line2 + "\n") # Optional: Add a second header line if needed
        self.output_text.insert(tk.END, header_separator + "\n")
        self.output_text.update()


        try:
            # Get the generator but skip the first yielded item (step 0)
            steps_generator = dijkstra_step_by_step(self.graph, source_node)
            next(steps_generator) # Skip the first item (step 0)

            for step, n_prime, distances, previous in steps_generator:
                # Format N' as {u,x,y}
                ...
                for node in nodes:
                    ...
                    # Remove trailing '.0' for integer distances to match image (e.g., 2.0 becomes 2)
                    if dist_str.endswith('.0') and dist_str != 'inf':
                         dist_str = dist_str[:-2]

                    cell_content = f"{dist_str},{pred_val}"

                    # Add spaces to pad the cell content to the column width
            ...
            # This will happen if the generator was empty after skipping the first item
            self.output_text.insert(tk.END, "No steps to display after initial state.\n")
            self.output_text.update()
        except Exception as e:
